 ...
 */
exports.registerAudit = function (schema, connection, auditComparator) {
    ...
    schema.post('remove', function (doc) {
        afterModifyingDocument('remove', doc, fieldsNotToAudit, connection, auditComparator);
    });

    schema.post('save', function (doc) {
        afterModifyingDocument('save', doc, fieldsNotToAudit, connection, auditComparator);
    });
 ...
 */
function afterModifyingDocument(event, doc, fieldsNotToAudit, connection, auditComparator) {
    ...
    memory.get(doc.id, function (err, oldObject) {
        ...
        var _callback = function (result, operation) {
            ...
            if (result) {
                // Create audit object
                var audit = new Audit(connection)();
